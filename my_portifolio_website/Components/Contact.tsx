...
export default function Contact({handleSubmit , status} : Props) {
    return(
        ...
        <form onSubmit={(event)=>handleSubmit(event)} className="m-2 flex flex-col p-2">
          ...
          <span className="mt-5 self-center">
            <input type={"submit"} disabled={status.length > 0} className="rounded-full border-2 border-gray-800 p-2 px-3 text-center text-xl hover:cursor-pointer hover:bg-purple-300 disabled:bg-gray-500 disabled:cursor-not-allowed">
            </input>
          </span>
          <div>
          
            {status == "Sending" ? (
                <svg role="status" className="h-10 ml-auto mr-auto mt-2 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
